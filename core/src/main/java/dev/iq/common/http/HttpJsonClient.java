...
import dev.iq.common.io.pipe.Pipes;
import dev.iq.common.log.Log;
import dev.iq.common.serde.JsonSerde;
...
/**
 * HTTP client to facilitate working with REST calls using JSON.  The Gson library
 * is used for serde.
 */
...
public final class HttpJsonClient {
     ...
     */
    public static int getFormUrlEncodedNoReply(final URI uri, final Map<String, String> headers, final Map<String, String> params) {
     ...
     */
    public static int postFormUrlEncodedNoReply(final URI uri, final Map<String, String> headers, final Map<String, String> params) {
    ...
    public static <R, T> T postJsonWithReply(final URI uri, final Map<String, String> headers, final R requestBody, final Class<T> target) {
    ...
    private static <T> T executeWithReply(final Supplier<HttpRequest> fx, final Class<T> target) {
        ...
        if (target == String.class) {
            return (T) Pipes.charsSupplier().read(() -> new InputStreamReader(response.body(), Charset.defaultCharset()));
        }
        ...
        return Io.withReturn(() -> {
            try (final var reader = HttpResponseCharset.getReader(response)) {
    ...
    private static HttpResponse<InputStream> execute(final Supplier<HttpRequest> fx) {
        ...
        try (final var client = HTTP_BUILDER.build()) {
            final var response = Io.withReturn(
    ...
    private static HttpRequest toGetRequest(final URI uri, final Map<String, String> headers) {
        ...
        Log.debug(HttpJsonClient.class, () -> "HTTP request sent: %n%s%n".formatted(HttpPrettyPrinter.toString(request, Optional.empty())));
        return request;
    }
     ...
     */
    private static HttpRequest toGetFormUrlEncoded(final URI uri, final Map<String, String> headers, final Map<String, String> params) {
     ...
     */
    private static HttpRequest toPostFormUrlEncoded(final URI uri, final Map<String, String> headers, final Map<String, String> params) {
    ...
    private static String toUrlEncoded(final Map<String, String> params) {
        ...
        return params.entrySet()
            .stream()
            .map(e -> "%s=%s".formatted(e.getKey(), URLEncoder.encode(e.getValue(), Charset.defaultCharset())))
     ...
     */
    private static <R> HttpRequest toJsonRequest(final URI uri, final Map<String, String> headers, final R requestBody) {
    ...
    private static void populateHeaders(final Map<String, String> headers, final HttpRequest.Builder builder) {
        ...
        headers.forEach((k, v) -> builder.header(k, URLEncoder.encode(v, Charset.defaultCharset())));
    }
}
